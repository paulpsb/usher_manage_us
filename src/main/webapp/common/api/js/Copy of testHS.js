		...
		*/
		...
		function startTest(mode1){
	        	...
	        	if(_rs.error == API_SUCCESS){
					...
					test_interval = setInterval(function(){
						...
						if(time1>=0){
							...
							function resultTestHSCallBack(_rs){
							...
							requestApi('POST', apiList('resultTestHS'), param_3, resultTestHSCallBack);
						}
					}, 1000);
	        ...
	        function xhr(data, callback) {
	            ...
	            request.open('POST', location.protocol+"//"+location.host + '/api/async/'+apiList('insertTestHS'));
	            request.send(data);
	        }
