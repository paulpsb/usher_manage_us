	...
	showGD: function(){
		...
		function startTest(mode1){
	        	...
	        	if(_rs.error == API_SUCCESS){
					...
					test_interval = setInterval(function(){
						...
						if(time1>=0){
							...
							function resultTestMACallBack(_rs){
							...
							requestApi('POST', apiList('resultTestMA'), param_3, resultTestMACallBack);
						}
					}, 1000);
	        ...
	        function xhr(data, callback) {
	            ...
	            request.open('POST', location.protocol+"//"+location.host + '/api/async/'+apiList('insertTestMA'));
	            request.send(data);
	        }
