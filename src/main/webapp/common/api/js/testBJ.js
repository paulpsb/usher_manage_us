var testBJ = {
	openWindow: function(){
		...
		function testBJCallBack(_rs){
			if(_rs.error == API_SUCCESS){
				var rsData = _rs.data.data_list;
				for(var i=0;i<rsData.length;i++){
					var opti = $('<option>').val(i).html(rsData[i].tbj_subject).appendTo(select);
					send_data.push(rsData[i]);
					
				}
				btn_send.on('click', function(){
					vp.close();
					testBJ.showBJ(send_data[select.val()]);
				});
				
			}
		}
		requestApi('POST', apiList('testBJ'), undefined, testBJCallBack);
		
	},
	showBJ: function(tbj_data){
		console.log(tbj_data);
		var tbj__subject = tbj_data.tbj_subject;
		var tbj__summary = tbj_data.tbj_data.tbj_summary;
		var tbj__word = tbj_data.tbj_data.tbj_word;
		...
		var title_p_input = $('<input type="text" name="tbj_subject" id="tbj_subject">').val(tbj__subject).css({height: '30px', width: '884px'}).appendTo(title_p);
		...
		function makeList(word_data, word_idx){
			...
			var span_txt1_input = $('<input type="text" name="tbj_word['+q_num+']">').val(word_data).css({width: '100%', height: '30px', lineHeight: '30px', border: 'none'}).appendTo(span_txt1);
			var span_txt2 = $('<span>', {'class':'txt2'}).css({width: '737px', display: 'block', float: 'left', lineHeight: '30px', marginRight: '14px'}).appendTo(li1);
			function makeList2(summary_data){
				var span_txt2_input = $('<input type="text" name="tbj_summary['+q_num+'][]">').val(summary_data).css({width: '100%', height: '30px', lineHeight: '30px', border: 'none'}).appendTo(span_txt2);
			}
			for(var i=0;i<tbj__summary[""+word_idx].length;i++){
				makeList2(tbj__summary[""+word_idx][i]);
			}
		}
		for(var a in tbj__word){
			makeList(tbj__word[a], a);
			q_num++;
		}
		...
		var span2_num = $('<span>', {'class': 'num'}).html("&nbsp").appendTo(li2);
		...
		function startTest(mode1){
		    ...
		    else if(mode1 == 'insert'){
		    	formData.append('inskey', tbj_data.tbj_no);
		    }
	        	...
	        	if(_rs.error == API_SUCCESS){
					...
					test_interval = setInterval(function(){
						...
						if(time1>=0){
							...
							function resultTestBJCallBack(_rs){
							...
							requestApi('POST', apiList('resultTestBJ'), param_3, resultTestBJCallBack);
						}
					}, 1000);
	        ...
	        function xhr(data, callback) {
	            ...
	            request.open('POST', location.protocol+"//"+location.host + '/api/async/'+apiList('insertTestBJ'));
	            request.send(data);
	        }
