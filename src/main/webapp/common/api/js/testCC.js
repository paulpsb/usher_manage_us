		...
		*/
		...
		function startTest(mode1){
	        	...
	        	if(_rs.error == API_SUCCESS){
					...
					test_interval = setInterval(function(){
						...
						if(time1>=0){
							...
							function resultTestCCCallBack(_rs){
							...
							requestApi('POST', apiList('resultTestCC'), param_3, resultTestCCCallBack);
						}
					}, 1000);
	        ...
	        function xhr(data, callback) {
	            ...
	            request.open('POST', location.protocol+"//"+location.host + '/api/async/'+apiList('insertTestCC'));
	            request.send(data);
	        }
