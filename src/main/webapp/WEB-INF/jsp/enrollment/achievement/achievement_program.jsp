...
jQuery(document).ready(function(){
	$.ajax({
		type : "POST",
		url : "/enrollment/achievement/getProgramList.do",
		data:{
			section:section,
		...
		success:function(data){
			...
			for(var i=date_count-1; i>=0; i--)
			{
				...
				if(arr_schedule.length == 0){
					vHtml += '			<th class="text-center table-info" style="width:100px;">&nbsp;</th>';
					column_length++;
				}else{
				...
				for(var k=0; k<arr_schedule.length; k++)
				{
					...
					}else if(cur_date < v_date)
					{
						...
						if(v_date < first_date)
						{
							...
							var r_idx = resultList.findIndex(t => t.date == v_date 
							...
							if(r_idx >= 0){
								...
								if(practice_type == "MOCK_TEST"){
									...
									vHtml += score;
									vHtml += '</td>';
									//vHtml += '			<td class="click_practice bg-green text-center" style="width:100px;">'+score+'</td>';
								}else{
									if(resultList[r_idx].pass_result){
										...
										vHtml += score;
										vHtml += '</td>';
										//vHtml += '			<td class="click_practice bg-green text-center" style="width:100px;">O('+score+')</td>';
									}else{
										...
										vHtml += score;
										vHtml += '</td>';
										//vHtml += '			<td class="click_practice bg-red text-center" style="width:100px;">X('+score+')</td>';
									}
								}
