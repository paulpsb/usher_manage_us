...
function form_search()
{
	$.ajax({
		type : "POST",
		url : "/notice/getNoticeStatsDailyOraganizationList.do",
		data:{
			stats_date:$("#search_date").val()
		},
		...
		for(var i=0; i<array_auth_oragnization.length; i++)
		{
			...
			if(array_auth_user_oragnization.length > 0)
			{
				...
				for(var j=0; j<array_auth_user_oragnization.length; j++)
				{
					...
					vHtml += '<a href="javascript:move_daily_user('+v_user_id+',\''+v_user_name+'\')" class="text-dark">';
...
function move_daily_user(v_user_id, v_user_nm)
{
	window.open("/notice/notice_stats_daily_user.do?stats_date="+$("#search_date").val()+"&&user_id="+v_user_id+"&&user_name="+v_user_nm,"notice_stats_daily_user");
}