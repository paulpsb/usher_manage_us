		...
		success:function(data){
			...
			for(var i=0; i<countList.length; i++)
			{
				...
				var array_result = resultList.filter(function(item, index){
					if(item.batch_finally_test_type == test_type && item.batch_finally_student_type == student_type && item.batch_finally_courses == batch_select_courses){
						return true;
					}
				});
				...
				for(var j=0; j<array_result.length; j++)
				{
					...
					if(j==0){
						if(batch_select_courses){
							batch_select_courses = batch_select_courses + "반";
						}else{
...
function go_modify()
{
	...
	window.open('http://batch.usher.co.kr/modify/modify_user.do?user_id='+user_id,'modify');
}