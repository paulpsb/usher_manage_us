...
var course_groups = [
	{batch_finally_test_type:"TOEFL", batch_finally_student_type:"SENIOR",batch_finally_course_group:"REGULAR",         test_type_value:"토플", value:"성인 정규"},
	{batch_finally_test_type:"TOEFL", batch_finally_student_type:"SENIOR",batch_finally_course_group:"LIGHT",           test_type_value:"토플", value:"성인 정규Light"},
	{batch_finally_test_type:"TOEFL", batch_finally_student_type:"SENIOR",batch_finally_course_group:"SINGLE",          test_type_value:"토플", value:"성인 단과"},
	{batch_finally_test_type:"TOEFL", batch_finally_student_type:"JUNIOR",batch_finally_course_group:"REGULAR_MON_FRI", test_type_value:"토플", value:"주니어 월수금"},
	{batch_finally_test_type:"TOEFL", batch_finally_student_type:"JUNIOR",batch_finally_course_group:"REGULAR_TUE_THU", test_type_value:"토플", value:"주니어 화목"},
	{batch_finally_test_type:"TOEFL", batch_finally_student_type:"JUNIOR",batch_finally_course_group:"SPECIAL",         test_type_value:"토플", value:"주니어 방학특강"},
	{batch_finally_test_type:"TOEIC", batch_finally_student_type:"SENIOR",batch_finally_course_group:"REGULAR",         test_type_value:"토플", value:"토익 종합반"},
	{batch_finally_test_type:"TOEIC", batch_finally_student_type:"SENIOR",batch_finally_course_group:"SINGLE",          test_type_value:"토플", value:"토익 단과반"}	
]

var array_batch_exam_month;
jQuery(document).ready(function(){
	var v_batch_schedule_id            = $("#search_batch_schedule_id").val();
	var v_batch_adviser_register_yn    = $("#search_batch_adviser_register_yn").val();
	var v_batch_adviser_register_date  = $("#search_batch_adviser_register_date").val();
	if(v_batch_schedule_id != "0" || v_batch_adviser_register_yn || v_batch_adviser_register_date){
		form_search();
	}else{
		search_batch_exam_month();
	}
});

function search_batch_exam_month()
{
...
function create_search_area()
{
	...
	vHtml += '		<input id="search_student_name" class="form-control">';
	vHtml += '	</div>';
	vHtml += '</div>';
	...
	vHtml += '	<div class="col-md-1">';
	vHtml += '		<select id="search_batch_exam_date" class="form-control">';
	for(var i=0; i<array_batch_exam_month.length; i++)
	{
		vHtml += '			<option value="'+array_batch_exam_month[i].semester_date+'">'+array_batch_exam_month[i].semester_date+'</option>';
	}
	vHtml += '		</select>';
	vHtml += '	</div>';
	...
	$('#search_batch_exam_date').change(function(e){
		form_search();
	});	
...
function create_table()
{
	...
	vHtml += '<div class="table-responsive text-center">';
	vHtml += '	<table id="example"  class="table table-bordered table-td-valign-middle m-b-0">';
	vHtml += '		<thead>';
	vHtml += '			<tr>';
	vHtml += '				<th class="text-center bg-info" style="width:40px;">No.</th>';
	vHtml += '				<th class="text-center bg-info" style="width:120px;">이름/상담카드</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">상담예약일</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">상담변경일</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">상담일</th>';
	vHtml += '				<th class="text-center bg-info" style="width:80px;">상담자</th>';
	vHtml += '				<th class="text-center bg-info" style="width:120px;">배치 반그룹</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">배치 반</th>';
	vHtml += '				<th class="text-center bg-info" style="width:250px;">상담특이사항</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">시작일</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">상담 당일 결과</th>';
	vHtml += '				<th class="text-center bg-info" style="width:200px;">학생 사유</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">등록일<br>등록예정일<br>고민기한</th>';
	vHtml += '				<th class="text-center bg-info" style="width:250px;">연락</th>';
	vHtml += '				<th class="text-center bg-info" style="width:100px;">실제등록여부</th>';
	vHtml += '				<th class="text-center bg-info" style="width:200px;">사유</th>';
	vHtml += '			</tr>';
	vHtml += '		</thead>';
	vHtml += '		<tbody id="result_list">';
	vHtml += '																																										';
	vHtml += '		</tbody>';
	vHtml += '	</table>';
	vHtml += '</div>';

	$("#div_detail").html(vHtml);
}
function form_search()
{
	...
	var v_batch_adviser_register_yn    = $("#search_batch_adviser_register_yn").val();
	var v_batch_adviser_register_date  = $("#search_batch_adviser_register_date").val();
	if(!v_batch_schedule_id) v_batch_schedule_id = "0";
	$.ajax({
		type : "POST",
		url : "/batch/getBatchStatsDailyList.do",
		data : {
			batch_exam_date:$("#search_batch_exam_date").val(),
			student_name:$("#student_name").val(),
			batch_finally_test_type:test_type,
			batch_finally_student_type:student_type,
			batch_finally_course_group:course_group,
			batch_schedule_id:v_batch_schedule_id,
			batch_adviser_register_yn:v_batch_adviser_register_yn,
			batch_adviser_register_date:v_batch_adviser_register_date
		},
		dataType : "json",
		success:function(data){
			var vHtml = "";
			...
			{
				var v_name = data[i].student_name;
				var v_type ="";
				var v_course_group ="";
				var v_course = data[i].batch_finally_courses;
				var v_register_date = data[i].batch_adviser_register_date;
				var v_register ="";
				
				var v_batch_finally_test_type    = data[i].batch_finally_test_type;
				var v_batch_finally_student_type = data[i].batch_finally_student_type;
				var v_batch_finally_course_group = data[i].batch_finally_course_group;

				var v_batch_finally_course_group_name = "";
				var a_idx = course_groups.findIndex(t => t.batch_finally_test_type == v_batch_finally_test_type && t.batch_finally_student_type == v_batch_finally_student_type && t.batch_finally_course_group == v_batch_finally_course_group);
				if(a_idx >= 0)
				{
					v_batch_finally_course_group_name = course_groups[a_idx].value;
					v_course_group = course_groups[a_idx].value;
					v_type = course_groups[a_idx].test_type_value
				}
				var nSeq = i + 1;
				

				
				vHtml += '<tr style="height:50px;">';
				vHtml += '<td>'+nSeq+'</td>';
				vHtml += '<td><a href="javascript:go_batch('+data[i].user_id+')">'+data[i].student_name+'</a></td>';
				vHtml += '<td>'+data[i].batch_resv_date+'</td>';
				vHtml += '<td>'+data[i].batch_change_date+'</td>';
				vHtml += '<td>'+data[i].batch_exam_date+'</td>';
				vHtml += '<td>'+data[i].adviser_name+'</td>';
				vHtml += '<td>'+v_batch_finally_course_group_name+'</td>';
				vHtml += '<td>'+data[i].batch_finally_courses+'</td>';
				vHtml += '<td class="text-left">'+cfmNvl1(data[i].batch_adviser_advice)+'</td>';
				vHtml += '<td>'+cfmNvl1(data[i].attend_start_date)+'</td>';
				if(data[i].batch_adviser_register_yn == "Y"){
					vHtml += '<td class="bg-green-lighter">당일 등록</td>';
					v_register ="당일 등록";
				}else if(data[i].batch_adviser_register_yn == "S"){
					vHtml += '<td class="bg-blue-lighter">등록예정</td>';
					v_register ="등록예정";
				}else if(data[i].batch_adviser_register_yn == "Q"){
					vHtml += '<td class="bg-yellow-lighter">고민</td>';
					v_register ="고민";
				}else{
					vHtml += '<td class="bg-red-lighter">거절</td>';
					v_register ="거절";
				}
				vHtml += '<td class="text-left">'+cfmNvl1(data[i].batch_adviser_not_register_desc)+'</td>';
				vHtml += '<td>'+data[i].batch_adviser_register_date+'</td>';
				vHtml += '<td>';
				if(data[i].batch_send_kakao_before_yn == "Y"){
					vHtml += '<button type="button" class="btn btn-success m-5" style="width:70px;height:70px;" onclick="go_send_kakao('+data[i].batch_result_id+',\'send_kakao_before\',\''+v_name+'\',\''+v_type+'\',\''+v_course_group+'\',\''+v_course+'\',\''+v_register_date+'\',\''+v_register+'\')" >하루 전<br>알림톡<br>전송</button>';
				}else{
					vHtml += '<button type="button" class="btn btn-grey m-5" style="width:70px;height:70px;" onclick="go_send_kakao('+data[i].batch_result_id+',\'send_kakao_before\',\''+v_name+'\',\''+v_type+'\',\''+v_course_group+'\',\''+v_course+'\',\''+v_register_date+'\',\''+v_register+'\')">하루 전<br>알림톡<br>전송</button>';
				}
				if(data[i].batch_send_kakao_current_yn == "Y"){
					vHtml += '<button type="button" class="btn btn-success m-5" style="width:70px;height:70px;" onclick="go_send_kakao('+data[i].batch_result_id+',\'send_kakao_current\',\''+v_name+'\',\''+v_type+'\',\''+v_course_group+'\',\''+v_course+'\',\''+v_register_date+'\',\''+v_register+'\')">당일<br>알림톡<br>전송</button>';
				}else{
					vHtml += '<button type="button" class="btn btn-grey m-5" style="width:70px;height:70px;" onclick="go_send_kakao('+data[i].batch_result_id+',\'send_kakao_current\',\''+v_name+'\',\''+v_type+'\',\''+v_course_group+'\',\''+v_course+'\',\''+v_register_date+'\',\''+v_register+'\')">당일<br>알림톡<br>전송</button>';
				}
				if(data[i].batch_send_phone_yn == "Y"){
					vHtml += '<button type="button" class="btn btn-success m-5" style="width:70px;height:70px;" onclick="go_send_phone('+data[i].batch_result_id+',\'send_phone\')">전화<br>연락</button>';
				}else{
					vHtml += '<button type="button" class="btn btn-grey m-5" style="width:70px;height:70px;" onclick="go_send_phone('+data[i].batch_result_id+',\'send_phone\')">전화<br>연락</button>';
				}
				vHtml += '</td>';
				if(data[i].batch_finally_register_yn == "Y")
				{
					vHtml += '<td class="bg-green-lighter">등록</td>';
				}else if(data[i].batch_finally_register_yn == "S"){
					if(data[i].batch_adviser_register_yn == "S"){
						vHtml += '<td class="bg-blue-lighter">예정 대기</td>';
					}else if(data[i].batch_adviser_register_yn == "Q"){
						vHtml += '<td class="bg-yellow-lighter">고민 대기</td>';
					}
				}else{
					vHtml += '<td class="bg-red-lighter">미등록</td>';
				}
				if(data[i].batch_finally_register_yn != "Y")
				{
					vHtml += '<td><p class="text-left">'+cfmNvl1(data[i].batch_finally_not_register_desc)+'</p>';
					vHtml += '<a href="javascript:go_finally_not_register_desc('+data[i].batch_result_id+')" class="btn btn-primary btn-xs">사유등록</a>';
					vHtml += '</td>';
				}else{
					vHtml += '<td class="text-left"></td>';
				}
...
var batch_result_id;
var batch_send_type;
var batch_send_argument;
var batch_send_desc;

function go_finally_not_register_desc(v_batch_result_id)
{
	$.ajax({
		type : "POST",
		url : "/batch/getBatchResultAsID.do",
		data : {
			id : v_batch_result_id
		},
		dataType : "json",
		success:function(data){
			batch_result_id = v_batch_result_id;
...
function save_finally_not_register_desc()
{
	$.ajax({
		type : "POST",
		url : "/batch/updateBatchResultFinallyNotRegisterAsId.do",
		data : {
			id : batch_result_id,
...
function go_send_kakao(v_batch_result_id, v_batch_send_type, v_name, v_type, v_course_group, v_course, v_register_date, v_register)
{
	$.ajax({
		type : "POST",
		url : "/batch/getBatchResultSendList.do",
		data : {
			batch_result_id : v_batch_result_id,
			batch_send_type:v_batch_send_type
		},
		dataType : "json",
		success:function(data){
			batch_result_id = v_batch_result_id;
			batch_send_type = v_batch_send_type;
			batch_send_argument = v_name+"|"+v_type+"|"+v_course_group+"|"+v_course+"|"+v_register+"|"+v_register_date+"|"+v_register;
			if(data.length > 0)
			{
				batch_send_desc = data[0].batch_send_desc;
				$("#send_kakao_desc").val(data[0].batch_send_desc);
				$("#send_kakao_sender").html(data[0].batch_send_date+" "+data[0].batch_send_name);
				$("#btn_send_kakao").show();
			}else{
				batch_send_desc = getTextKakao(batch_send_type, v_name, v_type, v_course_group, v_course, v_register_date, v_register);
				$("#send_kakao_desc").val(batch_send_desc);
				$("#send_kakao_sender").html("");
				$("#btn_send_kakao").show();
			}
			$("#modal-send-kakao").modal();
		},
		error:function(event){				
			alert("잠시후 다시 시도 바랍니다.");
		}
...
function getTextKakao(v_Gubun, v_name, v_type, v_course_group, v_course, v_register_date, v_register)
{
	var vHtml = "";
	if(v_Gubun == "send_kakao_before"){
		vHtml += '안녕하십니까 어셔어학원입니다.\n';
		vHtml += v_name+' 님과의 상담결과에 따라 배치되신\n';
		vHtml += v_type+' '+v_course_group+' '+v_course+' 수강에 대한 안내로 연락드립니다.\n';
		vHtml += '상담 당일, 등록여부 조사에서 '+v_register+'을 선택해주셨고\n';
		vHtml += v_register_date+' 일 까지 '+v_register+' 하신다고\n';
		vHtml += '응답해주셔서 하루 전 확인차 연락드립니다.\n';
		vHtml += '내일 다시 전화 드리도록 하겠습니다.\n';
		vHtml += '궁금하신 사항은 주말을 제외한 평일 오전 9시부터 오후 7시까지 어셔어학원 카카오톡 채널이나,\n';
		vHtml += '어셔어학원 전화번호 02-595-5679로 문의 해주시면 성실히 응대하겠습니다.\n';
	}else{
		vHtml += '안녕하십니까 어셔어학원입니다.\n';
		vHtml += v_name+' 님과의 상담결과에 따라 배치되신\n';
		vHtml += v_type+' '+v_course_group+' '+v_course+' 수강에 대한 안내로 연락드립니다.\n';
		...
		vHtml += v_register_date+' 일 까지 '+v_register+' 하신다고\n';
		vHtml += '응답해주셨습니다.\n';
		vHtml += '오늘 등록에 관련하여 안내 전화를 드릴 예정입니다.\n';
		vHtml += '궁금하신 사항은 주말을 제외한 평일 오전 9시부터 오후 7시까지 어셔어학원 카카오톡 채널이나,\n';
		vHtml += '어셔어학원 전화번호 02-595-5679로 문의 해주시면 성실히 응대하겠습니다.\n';
	}
...
function save_send_kakao()
{
	if(!confirm("전송하시겠습니까?")) return;
	$.ajax({
		type : "POST",
		url : "/batch/insertBatchResultSend.do",
		data : {
			batch_result_id : batch_result_id,
			batch_send_type:batch_send_type,
			batch_send_desc:batch_send_desc,
			batch_send_argument:batch_send_argument
		},
		success:function(data){
			alert("전송하였습니다.");
			$("#modal-send-kakao").modal("hide");
			form_search();
		},
		error:function(event){				
			alert("잠시후 다시 시도 바랍니다.");
...
function go_send_phone(v_batch_result_id, v_batch_send_type)
{
	$.ajax({
		type : "POST",
		url : "/batch/getBatchResultSendList.do",
		...
		success:function(data){
			...
			for(var i=0; i<data.length; i++)
			{
				...
				vHtml += '	<textarea class="form-control" rows="3" readonly>'+data[i].batch_send_desc+'</textarea>';
				vHtml += '</div>';
				vHtml += '<div class="col-12 text-right">';
				vHtml += '	<p>'+data[i].batch_send_date+" "+data[i].batch_send_name+'</p>';
				vHtml += '</div>';
			}
			$("#div_send_mobile").html(vHtml);
			$("#send_phone_desc").val("");
			$("#modal-send-mobile").modal();
		},
		error:function(event){				
			alert("잠시후 다시 시도 바랍니다.");
		}
...
function save_send_mobile()
{
	if(!confirm("전화통화내역을 입력하시겠습니까?")) return;
	$.ajax({
		type : "POST",
		url : "/batch/insertBatchResultSend.do",
		data : {
			batch_result_id : batch_result_id,
			batch_send_type:batch_send_type,
			batch_send_desc:$("#send_phone_desc").val(),
			batch_send_argument:batch_send_argument
		},
		success:function(data){
			alert("전화통화내역을 입력하였습니다.");
			$("#modal-send-mobile").modal("hide");
			form_search();
		},
		error:function(event){				
			alert("잠시후 다시 시도 바랍니다.");
