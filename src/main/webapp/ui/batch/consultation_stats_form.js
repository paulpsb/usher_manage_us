jQuery(document).ready(function(){
	$.ajax({
		type : "POST",
		url : "/batch/getBatchStatsList.do",
		data:{
			semester_date:$("#semester_date").val(),
			batch_finally_student_type:$("#batch_finally_student_type").val(),
			semester_registration_date:$("#semester_registration_date").val(),
			finally_register:$("#finally_register").val(),
			finally_register_detail:$("#finally_register_detail").val()
		},
		success:function(data){
			...
			for(var i=0; i<data.length; i++)
			{
				...
				vHtml += '<td>'+data[i].course_group_name+'</td>';
				vHtml += '<td>'+data[i].course_name+'</td>';
				vHtml += '<td>'+data[i].batch_adviser_advice+'</td>';
				vHtml += '<td>'+data[i].attend_start_date+'</td>';
				vHtml += '<td>'+data[i].batch_adviser_register+'</td>';
				vHtml += '<td>'+data[i].batch_adviser_not_register_desc+'</td>';
				vHtml += '<td>'+data[i].batch_adviser_register_date+'</td>';
				vHtml += '<td>'+data[i].student_send+'</td>';
				vHtml += '<td>'+data[i].real_register+'</td>';
				vHtml += '<td>&nbsp;</td>';
				vHtml += '</tr>';
			}
