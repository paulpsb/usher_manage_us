...
function search_schedule()
{
	$.ajax({
		type : "POST",
		url : "/test/getPracticeScheduleVocaIntervalCourseGroupPracticeTypeList.do",
		data:{
			section:"VOCA",
		...
		success:function(data){
			...
			schedule_list = data.scheduleList1;
			schedule_list1  = data.scheduleList2;
			if(schedule_list1.length > 0){
				var v_schedule_title = "";
				for(var i=0; i<schedule_list1.length; i++)
				{
					var v_book = schedule_list1[i].book;
					var v_day  = schedule_list1[i].volume;
				...
				var to_time = schedule_list[0].practice_schedule_current_time;
				...
				var practice_schedule_start_yn = schedule_list[0].practice_schedule_start_yn;
				var interval_time = schedule_list[0].interval_time;
				if(interval_time == 0){
					interval_time = 120;
				}
				if(practice_schedule_start_yn == "Y"){
					
					$("#start_time").html(schedule_list[0].practice_schedule_start_time);
					$("#end_time").html(schedule_list[0].practice_schedule_end_time);
					...
					exam_remain_time = schedule_list[0].remain_time;
					if(exam_remain_time <= 0)
					{
						stopExam();
					}else{
...
function search_student_result()
{
	$.ajax({
		type : "POST",
		url : "/test/getPracticeResultVocaIntervalCourseGroupList.do",
		data:{
			course_group_id:course_group_id,
			date:cfmGetToDate()
		},
...
function create_student_result()
{
	...
	for(var i=0; i<schedule_list1.length; i++)
	{
		var exam_count = schedule_list1[i].exam_count;
		var total_exam_count = schedule_list1[i].total_score;
	...
	vHtml += '		<th class="text-center">반</th>';
	...
	for(var k=0; k<enrollment_list.length; k++)
	{
		...
		vHtml += '	<td class="text-center"'+rowspan+'><a href="javascript:go_achieve(\''+v_course_id+'\');">'+enrollment_list[k].course_name+'반</td>';
...
function startExam()
{
	$.ajax({
		type : "POST",
		url : "/test/updatePracticeScheduleIntervalCourseGroup.do",
		data:{
			section:"VOCA",
			practice_type:"VOCA_INTERVAL",
			course_group_id:course_group_id,
