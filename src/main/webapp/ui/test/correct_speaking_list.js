...
jQuery(document).ready(function(){
	...
	$('#book,#student_pen_yn,#answer_rublic_yn,#answer_spk_yn').change(function(e){
		form_search();
	});	
	form_search();
...
function search_list(vPage)
{
	...
	$.ajax({
		type : "POST",
		url : "/test/getCorrectSpeakingList.do",
		data:{
			section:"SPEAKING",
			...
			answer_spk_yn:$("#search_answer_spk_yn").val(),
			course_id:course_id,
			page:vPage,
			row_num:row_num
		},
		success:function(data){
			var total_count = data.correctSpeakingCount.total_count;
			var total_page = Math.ceil(total_count/row_num);
			
			var correctSpeakingList = data.correctSpeakingList;
			var vocaSpeechList = data.vocaSpeechList;
			
			var vHtml = "";
			for(var i=0; i<correctSpeakingList.length; i++){
				var v_book = correctSpeakingList[i].book;
				vHtml += "<tr>";
				vHtml += "<td class='text-center'>"+v_book+"</td>";
				vHtml += "<td class='text-center'>"+correctSpeakingList[i].article+"</td>";
				vHtml += "<td>";
				vHtml += "<a href='javascript:open_speaking("+correctSpeakingList[i].id+")'>";
				vHtml += correctSpeakingList[i].question;
				vHtml += "</a>";
				vHtml += "</td>";
				vHtml += "<td class='text-center'>"+correctSpeakingList[i].student_name+"</td>";
				vHtml += "<td class='text-center'>"+correctSpeakingList[i].date+"</td>";
				vHtml += "<td class='text-center'>"+correctSpeakingList[i].student_pen_yn+"</td>";
				vHtml += "<td class='text-center'>"+correctSpeakingList[i].answer_rublic_yn+"</td>";
				vHtml += "<td class='text-center'>"+correctSpeakingList[i].answer_spk_yn+"</td>";
				vHtml += "<td class='text-center'>"+correctSpeakingList[i].student_note_yn+"</td>";
				vHtml += "</tr>";
			}
...
function open_speaking(practice_result_id){
	window.open("/test/correct/correct_speaking.do?id="+practice_result_id, "speaking");
