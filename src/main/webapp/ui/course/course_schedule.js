...
function form_search()
{
	$.ajax({
		...
		success:function(data){
			...
			$("#select_course_title").html("Syllabus ("+courseInfo.semester_date+")");
			...
			for(var i=0; i<arr_date.length; i++)
			{
				...
				for(var j=0; j<typeList.length; j++)
				{
					...
					var vProgramUse = typeList[j].program_use;
					var arr_schedule = scheduleList.filter(function(item, index){
						if(item.date == vDate && item.practice_type == vPracticeType){
							return true;
						}
					});
					
					vHtml += '<td class="droppable text-center bg-white">';
					vHtml += '	<h5>'+typeList[j].practice_name+'</h5>';
					vHtml += '	<input type="hidden" class="schedule_date" value="'+vDate+'">';
					vHtml += '	<input type="hidden" class="homework_date" value="'+hDate+'">';
					...
					vHtml += '	<input type="hidden" class="program_use" value="'+vProgramUse+'">';
					...
					if(arr_schedule.length > 0)
					{
						for(var k=0; k<arr_schedule.length; k++)
						{
							...
							vHtml += 	'	<input type="hidden" id="schedule_date" name="schedule_date" value="'+vDate+'">';
							vHtml += 	'	<input type="hidden" id="schedule_date" name="homework_date" value="'+hDate+'">';
							...
							vHtml += '		<select id="start_paragraph" name="start_paragraph">';
							for(var t=0; t<=10; t++)
							{
								var selected = "";
							...
							select_display = "style='display:none;'";
							var v_section = $("#search_section option:checked").text();
							//if((v_section == "GRAMMAR" || v_section == "READING" || v_section == "LISTENING") && vPracticeType=="MOCK_TEST")
							if(vProgramUse == "Y")
							{
								if(vPracticeType != "VOCA"){
									select_display = "";
								}
							...
							if(vPracticeType == "VOCA_INTERVAL"){
								...
								vHtml += '<select id="exam_count" name="exam_count" style="display:none;"><option value="0"></option></select>';
								
							}
							if(vPracticeType == "TENTIMES"){
								vHtml += '		시험횟수 : <select id="exam_time" name="exam_time">';
								for(var t=1; t<=10; t++)
								{
									var selected = "";
									if(t == arr_schedule[k].exam_time) selected = "selected";
									vHtml += '			<option value="'+t+'" '+selected+'>'+t+'번</option>';
								}
								vHtml += '		</select>';
...
function initEvent()
{
	...
	$(".droppable").droppable({
		drop: function (event, ui) {
			...
			var hDate         = $(this).find(".homework_date").val();
			...
			var vProgramUse   = $(this).find(".program_use").val();
			
			var vSection = $("#search_section option:checked").text();
			var vProblem = $("#search_book option:checked").text();
			if(vDate < cur_date) return;
			
			vHtml += '<div class="section-drog">';
			vHtml += '	<input type="hidden" name="schedule_date" value="'+vDate+'">';
			vHtml += '	<input type="hidden" name="homework_date" value="'+hDate+'">';
			...
			vHtml += '	<div '+select_display+'>';	
			vHtml += '	<select name="start_paragraph">';
			for(var t=0; t<=10; t++)
			{
				if(t == 0){
			...
			var vExamTime = 10;
			var vExamCount = 15;
			//if((vSection == "GRAMMAR" || vSection == "READING" || vSection == "LISTENING") && vPracticeType=="MOCK_TEST")
			if(vProgramUse == "Y")
			{
				if(vSection != "VOCA")
				{
			...
			//$(this).append("<div class='section-drog'>"+ui.draggable.html()+"</div>");
			$(this).append(vHtml);
			//ui.draggable.hide();
	...
	$(".section-value").dblclick(function(){
		...
		if(vDate < cur_date){
			alert("이전일정 또는 당일일정은 삭제하실 수 없습니다.");
			return;
		}
...
function form_save()
{
	...
	$(".section-drog").each(function(index) {
		...
		objSchedule.score_type = "SCORE";
		objSchedule.name = $(this).find("input[name='practice_name']").val();
		objSchedule.total_score = $(this).find("input[name='total_score']").val();
		objSchedule.life_cycle = "CREATED_NEW";
		objSchedule.date = $(this).find("input[name='schedule_date']").val();
		objSchedule.homework_date = $(this).find("input[name='homework_date']").val();
	...
	$(".section-value").each(function(index) {
		...
		if(old_start_paragraph != start_paragraph || old_end_paragraph != end_paragraph || old_exam_time != exam_time || old_exam_count != exam_count)
		{
