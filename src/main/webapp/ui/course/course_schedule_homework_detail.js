...
{
	var arr_schedule = schedule.split(",");
	var sHtml = "";
	for(var i=arr_schedule.length-1; i>=0; i--)
	{
		if(arr_schedule[i] > cur_date) continue;
...
function form_search()
{
	$.ajax({
		type : "POST",
		url : "/course/getScheduleHomeworkDetailList.do",
		data:{
			course_id:course_id,
		...
		success:function(data){
			...
			$("#select_course_title").html("과제(개인별) ("+courseInfo.semester_date+")");
			...
			vHtml += create_schedule_course(courseInfo, typeList, scheduleList);
			
			for(var j=0; j<enrollmentList.length; j++)
			{
				vHtml += create_schedule_enrollment(enrollmentList[j], courseInfo, typeList, scheduleList);
			}
			vHtml += '</tbody>';
...
function create_schedule_course(courseInfo, typeList, scheduleList)
{
	var vHtml = "";
	var vCur_date = courseInfo.cur_date;
	//전체인경우
	vHtml += '	<tr>';
	vHtml += '		<th class="redips-mark text-center bg-black text-white" >'+course_name+'<br>반 과제</th>';
	for(var i=0; i<typeList.length; i++)
	{
		var vPracticeType = typeList[i].practice_type;
		var vPracticeName = typeList[i].practice_name;
		var vParagraphUse = cfmNvl1(typeList[i].paragraph_use);
		var vTypeComment  = cfmNvl1(typeList[i].type_comment);
		var arr_schedule = scheduleList.filter(function(item, index){
			if(item.practice_type == vPracticeType && item.course_enrollment_id == 0){
				return true;
			}
		});
		...
		for(var k=0; k<arr_schedule.length; k++)
		{
			...
			vHtml += '	<div class="bg-indigo text-white mb-2">';
			vHtml += '		<p style="margin-bottom:0px;">'+vTitle+'</p>';
			var select_display = "";
			
			//시험시간
			vHtml += '		시험시간 : '+arr_schedule[k].exam_time+'분';
			//시험횟수
			if(vPracticeType == "TENTIMES"){
				vHtml += '		시험횟수 : '+arr_schedule[k].exam_count+'회<br>';
			}
			...
			vHtml += '		문단 : '+arr_schedule[k].start_paragraph+'~'+arr_schedule[k].end_paragraph;
			vHtml += '	</div>';
			
			
			vHtml += '	<div>';
			vHtml += '	구분 : ';
			if(arr_schedule[k].test_type == "ALL")  vHtml += '전체 학생';
			if(arr_schedule[k].test_type == "FAIL") vHtml += '불합격 학생';
			vHtml += '</div>';
...
function create_schedule_enrollment(enrollmentInfo, courseInfo, typeList, scheduleList)
{
	var vHtml = "";
	var vCourse_enrollment_id = enrollmentInfo.course_enrollment_id;
	var vCur_date = courseInfo.cur_date;
	//전체인경우
	vHtml += '	<tr>';
	vHtml += '		<th class="redips-mark text-center bg-black-transparent-5 text-white" >'+enrollmentInfo.last_name+enrollmentInfo.first_name+'</th>';
	for(var i=0; i<typeList.length; i++)
	{
		var vPracticeType = typeList[i].practice_type;
		var vPracticeName = typeList[i].practice_name;
		...
		var vTypeComment  = cfmNvl1(typeList[i].type_comment);
		var arr_schedule = scheduleList.filter(function(item, index){
			if(item.practice_type == vPracticeType && item.course_enrollment_id == vCourse_enrollment_id){
				return true;
			}
		});
		if(cur_date == $("#search_schedule").val()){
		...
		vHtml += '	<input type="hidden" class="schedule_date" value="'+$("#search_schedule").val()+'">';
		vHtml += '	<input type="hidden" class="paragraph_use" value="'+vParagraphUse+'">';
		vHtml += '	<input type="hidden" class="course_enrollment_id" value="'+vCourse_enrollment_id+'">';
		vHtml += '	<input type="hidden" class="type_comment" value="'+vTypeComment+'">';
		vHtml += '	<input type="hidden" class="test_type" value="N">';
	...
	$(".section-value").dblclick(function(){
		...
		if(confirm("삭제하시겠습니까?")){
			var $_schedule = $(this);
			 $.ajax({
				type : "POST",
				url : "/course/updateScheduleHomeworkDetail.do",
				data:{
					id:vId,
					status:"INACTIVE"
				},
...
function form_save()
{
	...
	$(".section-value").each(function(index) {
	 ...
	 $.ajax({
			type : "POST",
			url : "/course/saveScheduleHomeworkDetail.do",
			data:{
...
{
	location.href="/course/course_schedule_homework.do?course_id="+course_id+"&&section="+$("#search_section option:checked").text();
}