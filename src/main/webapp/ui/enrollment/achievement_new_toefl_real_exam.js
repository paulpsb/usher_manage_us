...
function form_init()
{
	...
	$.ajax({
		type : "POST",
		url : "/enrollment/getAchieveToeflExamRealList.do",
		data:{
			course_id : $("#course_id").val()
		},
		success:function(data){
			//상단화면을 그려준다.
			create_top_menu(data.sectionList, data.practiceAllList, "exam", "toefl_real_exam", $("#course_id").val(), data.courseInfo.student_type);
			...
			vHtml += '			<th class="text-center table-info" colspan="5" style="width:'+(nWidth*5)+'px;">'+semesterInfo.date+' 실제토플 성적</th>';
			vHtml += '			<th class="text-center table-info" colspan="5" style="width:'+(nWidth*5)+'px;">'+semesterInfo.pre_date1+' 실제토플 성적</th>';
			vHtml += '			<th class="text-center table-info" colspan="5" style="width:'+(nWidth*5)+'px;">'+semesterInfo.pre_date2+' 실제토플 성적</th>';
			vHtml += '			<th class="text-center table-info" colspan="5" style="width:'+(nWidth*5)+'px;">'+semesterInfo.pre_date3+' 실제토플 성적</th>';
			vHtml += '			<th class="text-center table-info" colspan="5" style="width:'+(nWidth*5)+'px;">'+semesterInfo.pre_date4+' 실제토플 성적</th>';
			vHtml += '			<th class="text-center table-info" colspan="5" style="width:'+(nWidth*5)+'px;">'+semesterInfo.pre_date5+' 실제토플 성적</th>';
			vHtml += '		</tr>';
			vHtml += '		<tr>';
			...
			for(var i=0; i<enrollmentList.length; i++)
			{
				...
				for(var j=0; j<6; j++)
				{
					...
					var realList;
					if(j > 0){
						realList = eval("data.realList"+j);
					}else{
						realList = eval("data.realList");
					}
					arr_to_list = realList.filter(function(item, index){
					...
					if(j < 5){
						var realPrevList = eval("data.realList"+(j+1));
						arr_prev_list = realPrevList.filter(function(item, index){
							if(item.user_id == v_user_id){
								return true;
							}
						});
...
function add_toefl(v_user_id, v_date, v_user_nm)
{
	...
	$("#exam_user").html(v_user_nm+"("+v_exam_date+") 실전 시험");
	$.ajax({
		type : "POST",
