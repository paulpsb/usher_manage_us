...
jQuery(document).ready(function(){
	...
	var t_book = "actual_test";
	if(t_practice_type == "GRAMMAR_TRANSLATION"){
		var t_book = "sw19";
	}
	$.ajax({
		type : "POST",
...
function form_init()
{
	...
	$.ajax({
		...
		success:function(data){
			...
			{
				if(problemList[i].article) continue;
				var nCol = scheduleList.length;
				vHtml += '			<th class="text-center table-info" colspan="'+nCol+'" style="width:'+(nWidth*nCol)+'px;">'+problemList[i].volume+'</th>';
			}
			vHtml += '		</tr>';
			vHtml += '		<tr>';
			for(var i=0; i<problemList.length; i++)
			{
				...
				{
					vHtml += '			<th class="text-center table-info" style="width:'+nWidth+'px;">'+scheduleList[j]+'</th>';
					column_length++;
				}
			}
			vHtml += '		</tr>';
			...
			for(var i=0; i<enrollmentList.length; i++)
			{
				...
				{
					if(problemList[t].article) continue;
					var a_section       = $("#section").val();
					var a_practice_type = $("#practice_type").val();
					var a_book          = problemList[t].book;
					var a_volume        = problemList[t].volume;
					var a_group         = problemList[t].group;
					var a_article       = problemList[t].article;
					...
					for(var j=0; j<scheduleList.length; j++)
					{
						if(enrollmentList[i].class_gubun == "반이동"||enrollmentList[i].class_gubun == "환불"){
							...
							var t_idx2 = resultList.findIndex(t => t.section == a_section 
										...
										&& t.pass_result == false);
							if(t_idx1 >=0){
								var score1 = resultList[t_idx1].score;
								var score = "O";
			...
			$(".click_practice").click(function(e){
				...
				move_result(v_section, v_practice_type,v_course_enrollment_id, v_date, v_book, v_volume, v_group, v_article, 0);
			});
