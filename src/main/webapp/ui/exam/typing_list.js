...
function search_list(vPage)
{
	...
	$.ajax({
		...
		success:function(data){
			var total_count = data.typingCount.total_count;
			var total_page = Math.ceil(total_count/row_num);
			
			var typingList = data.typingList;
			
			var vHtml = "";
			for(var i=0; i<typingList.length; i++){
				vHtml += "<tr>";
				vHtml += "<td>"+cfmNvl1(typingList[i].section)+"</td>";
				vHtml += "<td>"+cfmNvl1(typingList[i].book)+"</td>";
				vHtml += "<td>"+cfmNvl1(typingList[i].volume)+"</td>";
				vHtml += "<td>"+cfmNvl1(typingList[i].group)+"</td>";
				vHtml += "<td>"+cfmNvl1(typingList[i].article)+"</td>";
				vHtml += "<td class='with-btn text-center' nowrap=''>";
				vHtml += "	<button type='button' class='btn btn-sm btn-primary m-r-2' onclick='modify_form(\""+typingList[i].id+"\")'>수정</button>";
				vHtml += "	<button type='button' class='btn btn-sm btn-primary m-r-2' onclick='modify_form_content(\""+typingList[i].id+"\")'>지문수정</button>";
				vHtml += "	<button type='button' class='btn btn-sm btn-primary m-r-2' onclick='delete_form(\""+typingList[i].id+"\")'>삭제</button>";
				vHtml += "</td>";
				vHtml += "</tr>";
			}
...
function modify_form(typing_id)
{
	$.ajax({
		type : "POST",
		url : "/exam/getTyping.do",
		data:{
			id:typing_id
		},
		success:function(data){
...
function modify_form_content(chain_id)
{
	$.ajax({
		...
		success:function(data){
			$("#typing_content_id").val(data.id);
			$("#contents").val(cfmNvl1(data.contents));
			$("#modal-typing-content").modal();
...
function save_form()
{
	...
	if($('#typing_id').val() == "0")
	{
		url = "/exam/insertTyping.do";
	}else{
		url = "/exam/updateTyping.do";
	}
	$.ajax({
		type : "POST",
		url :url,
...
{
	var url = "";
	url = "/exam/updateTypingContent.do";
	$.ajax({
		type : "POST",
		url :url,
		data:{
			id:$('#typing_content_id').val(),
			contents:$('#contents').val()
		},
		success:function(data){
			alert("저장하였습니다.");
			$("#modal-typing-content").modal("hide");
		},
		error:function(event){				
			alert("잠시후 다시 시도 바랍니다.");
		}
