...
var dashbordToeflExamNotMatching;
var dashboard_timer_id;
function search_dashboard_info()
{
	var end_time = "";
	...
	$.ajax({
		...
		success:function(data){
			...
			dashbordToeflExamNotMatching = data.dashbordToeflExamNotMatching;
...
function create_dashboard()
{
	...
	//col width 설정
	vHtml += '<table class="table table-bordered m-b-0" id="table_dashboard">';
	vHtml += '<colgroup>';
	...
	vHtml += '	</tr>';
	vHtml += '	<tr>';
	vHtml += '		<th>참강 OT 강의실</th>';
	vHtml += '		<td>';
	
	vHtml += '		</td>';
	for(var i=0; i<course_count; i++)
	{
		if(courseList[i].zoom_ot_url){
			vHtml += '		<td><a href="'+courseList[i].zoom_ot_url+'" target="_blank">'+courseList[i].zoom_ot_url+'</a></td>';
		}else{
			vHtml += '		<td>&nbsp;</td>';
		}
	}
	...
	*/
	...
	$('.show_today_new_student').click(function(e){
		if(show_today_new_student){
			show_today_new_student = false;
			$('.show_today_new_student').html('<i class="fas fa-lg fa-fw m-r-10 fa-arrow-circle-down"></i>');
			$('.show_today_new_student_detail').hide();
		}else{
			show_today_new_student = true;
			$('.show_today_new_student').html('<i class="fas fa-lg fa-fw m-r-10 fa-arrow-alt-circle-up"></i>');
			$('.show_today_new_student_detail').show();
		}
	});
...
function create_student()
{
	...
	if(show_private)
	{
		rate = Math.round(a_count/c_count * 100);
	}else{
		rate = Math.round(a_count/f_count * 100);
	}
	...
	var al_count = a_count - ac_count;
	var cl_count = c_count - cc_count;
	var fl_count = f_count - fc_count;
	
	if(show_private)
	{
		vHtml += '<td class="'+bg_text+'" title="최초 인원 '+f_count+'명 대비 오늘의 출석율 '+rate+'%">'+a_count+'/'+c_count;
		vHtml += '<br>(현강 : '+al_count+'/'+cl_count+')';
		vHtml += '<br>(참강 : '+ac_count+'/'+cc_count+')';
		vHtml += '</td>';
	}else{
		vHtml += '<td class="'+bg_text+'" title="최초 인원 '+f_count+'명 대비 오늘의 출석율 '+rate+'%">'+a_count+'/'+c_count+'/'+f_count;
		vHtml += '<br>(현강 : '+al_count+'/'+cl_count+'/'+fl_count+')';
		vHtml += '<br>(참강 : '+ac_count+'/'+cc_count+'/'+fc_count+')';
		vHtml += '</td>';
	}
	for(var i=0; i<course_count; i++)
	{
		...
		al_count = a_count - ac_count;
		cl_count = c_count - cc_count;
		fl_count = f_count - fc_count;
...
function create_new_student()
{
	...
	if(show_new_student)
	{
		vHtml += '<div class="show_today_new_student" style="float:right"><i class="fas fa-lg fa-fw m-r-10 fa-arrow-circle-up"></i></div>';
	}else{
		vHtml += '<div class="show_today_new_student" style="float:right"><i class="fas fa-lg fa-fw m-r-10 fa-arrow-circle-down"></i></div>';
		display = 'style="display:none;"';
	}
	vHtml += '</th>';
	var f_count = dashboardList.filter(function(item, index){
	...
	vHtml += '<tr class="show_today_new_student_detail" '+display+'>';
	vHtml += '<th>&nbsp;</th>';
	vHtml += '<td>&nbsp;</td>';
	for(var i=0; i<course_count; i++)
	{
...
function create_sylabus()
{
	...
	for(var i=0; i<course_count; i++)
	{
		...
		for(var j=0; j<f_array.length; j++)
		{
			...
			vHtml += '<a href="javascript:go_sylabus('+course_id+',\''+f_array[j].section+'\')">';
			vHtml += '<span class="text-dark">'+f_array[j].section_short+' : '+f_array[j].section_all_count+'/</span><span class="text-danger">'+f_array[j].section_all_not_count+'</span>';
			vHtml += '</a>';
		}	
		vHtml += '</td>';
	}
	...
	for(var i=0; i<course_count; i++)
	{
		...
		for(var j=0; j<f_array.length; j++)
		{
			...
			vHtml += '<a href="javascript:go_sylabus('+course_id+',\''+f_array[j].section+'\')">';
			vHtml += '<span class="text-dark">'+f_array[j].section_short+' : '+f_array[j].section_cur_count+'/</span><span class="text-danger">'+f_array[j].section_cur_not_count+'</span>';
			vHtml += '</a>';
		}	
		vHtml += '</td>';
	}
	...
	for(var i=0; i<course_count; i++)
	{
		...
		for(var j=0; j<f_array.length; j++)
		{
			...
			vHtml += '<a href="javascript:go_sylabus('+course_id+',\''+f_array[j].section+'\')">';
			vHtml += '<span class="text-dark">'+f_array[j].section_short+' : '+f_array[j].section_next_count+'/</span><span class="text-danger">'+f_array[j].section_next_not_count+'</span>';
			vHtml += '</a>';
		}	
...
function create_kakaosend()
{
	...
	vHtml += '<tr class="kakao" '+display+'>';
	vHtml += '<th>문자 발송';

	
	vHtml += '</th>';	
	var t_count = dashboardList.filter(function(item, index){
				if(item.current_status == "OK" && item.refund_status != "FULL_REFUND" && item.status == "PAID"){
					return true;
				}
		}).length;
	var s_count = dashboardList.filter(function(item, index){
				if(item.current_status == "OK" && item.refund_status != "FULL_REFUND" && item.status == "PAID" && item.kakao_send_yn == "Y"){
					return true;
				}
		}).length;
	if(t_count > 0)
	{
		if(t_count != s_count){
			vHtml += '<td class="text-center bg-red-lighter text-white">'+s_count+"/"+t_count+'</td>';
		}else{
			vHtml += '<td class="text-center bg-green-lighter text-white">'+s_count+"/"+t_count+'</td>';
		}
	}else{
		vHtml += '<td>&nbsp;</td>';
	}
	for(var i=0; i<course_count; i++)
	{
		...
		t_count = dashboardList.filter(function(item, index){
						if(item.course_id == course_id && item.current_status == "OK" && item.refund_status != "FULL_REFUND" && item.status == "PAID"){
							return true;
						}
				}).length;
		s_count = dashboardList.filter(function(item, index){
						if(item.course_id == course_id && item.current_status == "OK" && item.refund_status != "FULL_REFUND" && item.status == "PAID" && item.kakao_send_yn == "Y"){
							return true;
						}
				}).length;
		if(t_count > 0)
...
function create_notsend()
{
	...
	for(var i=0; i<course_count; i++)
	{
		...
		var array_not_send = dashboardList.filter(function(item, index){
				if(item.course_id == course_id && item.current_status == "OK" && item.refund_status != "FULL_REFUND" && item.status == "PAID" && item.kakao_send_yn != "Y"){
					return true;
				}
		});
		vHtml += '<td>';
		for(var j=0; j<array_not_send.length; j++)
		{
			if(j > 0 && j%5 ==0){
				vHtml += "<br>";
			}
			if(array_not_send[j].chamgang_yn == "Y"){
				vHtml += "<span class='text-blue'>"+array_not_send[j].name+'</span>&nbsp;&nbsp;';
			}else{
				vHtml += array_not_send[j].name+'&nbsp;&nbsp;';
			}
...
function create_notexamtoefl()
{
	...
	vHtml += '<tr class="notexamtoefl" '+display+'>';
	vHtml += '<th>모의토플 미매칭';
	vHtml += '</th>';	
	var t_count = dashbordToeflExamNotMatching.toefl_not_matching_count;
	if(t_count > 0)
	{
		vHtml += '<td class="bg-red-lighter"><a href="javascript:go_examtoefl()" class="text-white">'+t_count+'</a></td>';
	}else{
		vHtml += '<td>&nbsp;</td>';
	}
	for(var i=0; i<course_count; i++)
...
function go_sylabus(v_course_id, v_section)
{
	window.open("/course/course_schedule.do?course_id="+v_course_id+"&&section="+v_section, "sylabus");
}

function go_examtoefl()
{
	window.open("/enrollment/toefl_excel.do", "toefl_exam");
}