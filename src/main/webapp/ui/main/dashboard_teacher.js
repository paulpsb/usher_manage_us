...
function to_time_sec()
{
	...
	if(to_min == 0 && to_sec == 0){
		search_form();
	}
	$('#current_time').text(to_time);
}
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0; i<task_list.length; i++)
			{
				...
				vHtml += '	<div class="draggable section-drag bg-grey-darker" style="width:100%;padding:5px;">';
				...
				objAlaram.title = "TASK";
				objAlaram.content = task_list[i].task_title+' '+"작업시간을 설정하세요.";
			...
			for(var i=0; i<base_timetable.length; i++)
			{
				...
				for(var j=0; j<task_list.length; j++)
				{
					...
					vHtml += '</div>';
					if(is_check_practice){
						if(v_task_status != "RESPONSE" && v_task_status != "COMPLETE"){
							var objAlaram = Object();
							objAlaram.title = "TASK";
							objAlaram.content = task_list[j].task_title+' '+"작업을 완료하세요.";
							array_alaram.push(objAlaram);
		...
		drop: function (event, ui) {
			...
			$.ajax({
				...
				success:function(data){
					...
					var vHtml = "";
					vHtml += '<div class="col-3 style="padding:3px;">';
					vHtml += '<button id="task_'+task_id+'" type="button" class="btn btn-grey btn-xs btn-block" onClick="open_task('+task_id+')">';
					vHtml += task_title;
					vHtml += '</button>';
					vHtml += '</div>';
...
function task_save(v_status)
{
	$.ajax({
		...
		success:function(data){
			alert("저장하였습니다.");
			$("#task_"+task_id).removeClass("btn-grey");
			$("#task_"+task_id).removeClass("btn-yellow");
			$("#task_"+task_id).removeClass("btn-info");
			$("#task_"+task_id).removeClass("btn-danger");
			$("#task_"+task_id).removeClass("btn-success");
			if(v_status == "WORKING"){
				$("#task_"+task_id).addClass("btn-yellow");
			}else if(v_status == "STARTING"){
				$("#task_"+task_id).addClass("btn-yellow");
...
function open_alram()
{
	...
	var task_list = array_alaram.filter(function(item, index){
		if(item.title == "TASK"){
			return true;
		}
	});
		...
		success:function(data){
			...
			for(var i=0; i<scheduleDetailList.length; i++)
			{
				...
				if(scheduleDetailList[i].schedule_detail_reference_image){
					...
					vHtml += '				<div class="col-12 text-center mb-3"><img src="'+scheduleDetailList[i].schedule_detail_reference_image+'" style="border:1px solid #000;width:100%;"></div>';
