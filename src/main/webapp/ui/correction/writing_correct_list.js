...
function search_list(vPage)
{
	...
	$.ajax({
		type : "POST",
		url : "/correction/getWritingCorrectionList.do",
		data:{
			section:"WRITING",
		...
		success:function(data){
			...
			for(var i=0; i<resultList.length; i++){
				...
				var v_book = "독립형";
				if(resultList[i].book == "integrated") v_book = "통합형";
				...
				if(resultList[i].answer_rublic_yn == "Y"){
					...
					vHtml += "<td class='text-center text-danger'>"+resultList[i].answer_rublic_yn;
					vHtml += "</td>";
				}
				...
				if(resultList[i].answer_correct_pen_yn == "Y"){
					...
					var rublic_count = resultList[i].answer_correct_pen_count - resultList[i].answer_count;
					vHtml += "<td class='text-center text-success'>"+resultList[i].answer_correct_pen_yn+"<br>"+resultList[i].answer_correct_pen_date+"<br>"+resultList[i].answer_correct_pen_name+"<br>("+v_rublic_time+")<br>("+rublic_count+"자)</td>";
				}else{
...
function search_ta_list()
{
	$.ajax({
		type : "POST",
		url : "/correction/getWritingCorrectionPenCountList.do",
		data:{
			section:"WRITING",
...
{
	window.open("/correction/ta_correct_list.do?section=WRITING&&answer_correct_pen_user_id="+v_answer_correct_pen_user_id+"&&answer_correct_pen_date="+v_date, "correct_list");
}

function open_ta_correct_month(v_answer_correct_pen_user_id, v_month)
{
	window.open("/correction/ta_correct_list.do?section=WRITING&&answer_correct_pen_user_id="+v_answer_correct_pen_user_id+"&&answer_correct_pen_month="+v_month, "correct_list");
}