...
jQuery(document).ready(function(){
    ...
    $("#pen_comment_review").on("input", function(e) {
		$("#pen_comment_review_count").html($("#pen_comment_review").val().length);
	});
    
    $("#pen_comment_review_count").html($("#pen_comment_review").val().length);
    window.resizeTo(930,900);
...
function InsertHTML2(str)
{
	...
	var value;
	if(!text) 
		value = ' <strong><span style="color: rgb(255, 0, 0);">('+ str +')</span></strong>';
	else 
		value = '<strike><strong>'+text+'<strong></strike><strong><span style="color: rgb(255, 0, 0);"> ('+ str +') </span></strong>';
...
function savePen()
{
	if(!$("#pen_comment_review").val()){
		alert("총평을 입력하세요.");
		return;
	}
	
	if($("#pen_comment_review").val().length < 40){
		alert("총평을 40자 이상 입력하세요.");
		return;
	}

	var v_pen_desc = CKEDITOR.instances.writing_editor1.getData();
	v_pen_desc = v_pen_desc.replaceAll("#39;","'");
	v_pen_desc = v_pen_desc.replaceAll("&nbsp;","");
	v_pen_desc = v_pen_desc.replaceAll("\n"," ");
	var t_pen_desc = v_pen_desc.replace(/(<([^>]+)>)/ig,"");
	t_pen_desc = fnReplaceExp(t_pen_desc);
	for(var i=0; i<patternArray.length; i++)
	{
		var v_pattern_desc = patternArray[i].title+patternArray[i].description;
		var t_pattern_desc = fnReplaceExp(v_pattern_desc);
		console.log(t_pattern_desc);
		t_pen_desc = t_pen_desc.replace(t_pattern_desc,"");
	}
	t_pen_desc = fnReplaceExp(t_pen_desc);
	var answer_correct_pen_count = t_pen_desc.length;
	$.ajax({
		type : "POST",
		url : "/correction/correct/saveWritingTestPen.do",
		data:{
			correction_exams_answer_id:$("#correction_exams_answer_id").val(),
			pen_comment:CKEDITOR.instances.writing_editor1.getData(),
			pen_comment_review:$("#pen_comment_review").val(),
...
function fnReplaceExp(str){  
	if(!str) return "";
	
	var regExp = /[ \{\}\[\]\/?.,;:|\)*~`!^\-_+┼<>@\#$%&\'\"\\(\=’,“”]/gi;
	var target = str.replace(regExp, "");
