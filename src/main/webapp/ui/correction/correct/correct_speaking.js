...
function search_form()
{
	$.ajax({
		type : "POST",
		url : "/correction/correct/getSpeakingResult.do",
		data:{
		...
		success:function(data){	
			...
			$("#speaking_question").html("<b>Question : </b>"+resultInfo.question);
			$("#reg_date").html("<b>작성일 : </b>"+resultInfo.date);
			$("#speaking_exam_time").html("<b>남은제한시간 : </b>"+min+":"+sec);
			var audio_url = aws_url + resultInfo.answer;
			if(resultInfo.legacy_id > 0){
				audio_url = "http://usher.co.kr/helper/download.php?filen=" + resultInfo.answer;
			}
			...
			if(resultInfo.answer_student_pen_yn == "Y"){
				$("#speaking_user_date").html('<b>개인 딕테이션 작성 일시 : </b>'+resultInfo.answer_pen_date);
				$("#speaking_user_data").html(resultInfo.answer_pen.replace(/\n/g, '<br/>'));
			}else{
			...
			if(resultInfo.answer_correct_pen_yn == "Y"){
				$("#speaking_comment_date").html('<b>첨삭 작성 일시 : </b>'+penInfo.modified);
				$("#speaking_comment_writer").html('<b>첨삭자 : </b>'+penInfo.pen_name);
				
				var audio_url1 = aws_url + penInfo.pen_comment;
				if(resultInfo.legacy_id > 0){
					audio_url1 = "http://usher.co.kr/helper/download.php?filen=" + penInfo.pen_comment;
				}
				$("#speaking_comment_data").html("<audio src='"+audio_url1+"' style='width:100%;' controls>해당 브라우저는 audio 태그를 지원하지 않습니다.</audio>");
			}else{
				$("#speaking_comment_data").html('<b>음성 첨삭이 등록되어있지 않습니다.</b>');
			}
			...
			if(resultInfo.answer_rublic_yn == "Y" && resultInfo.answer_correct_pen_yn == "Y"){
				...
				for(var a=0; a<appealList.length; a++)
				{
					...
					if(appealList[a].appeal_answer_yn == "Y"){
						var appeal_url = aws_url + appealList[a].appeal_answer;
						vHtml += "<p style='font-size:18px;padding:0px 30px;'><b>이의신청 답변</b></p>";
						vHtml += "<p style='padding:0px 30px;'>";
						vHtml += "<audio src='"+appeal_url+"' style='width:100%;' controls>해당 브라우저는 audio 태그를 지원하지 않습니다.</audio>";
						vHtml += '</p>';
			...
			if(resultInfo.answer_student_note_yn == "Y"){
				$("#speaking_note_date").html('<b>오답노트 작성 일시 : </b>'+noteInfo.modified);
...
function click_rubric()
{
	...
	window.open('/correction/correct/correct_speaking_rubric.do?id=' + $("#correction_exams_answer_id").val(),'correct_rubric',options);
}

function click_comment_spk()
{
	var options = 'toolbar=no,location=no,width=500,height=230,resizable=no,scrollbars=no,status=no';
	window.open('/correction/correct/correct_speaking_pen.do?id=' + $("#correction_exams_answer_id").val(),'correct_pen',options);
}
function open_comment(v_id)
{
	var options = 'toolbar=no,location=no,width=800,height=560,resizable=no,scrollbars=no,status=no';
	window.open('/correction/correct/correct_speaking_comment.do?id=' + v_id,'correct_comment',options);
...
function open_user_appeal(v_id)
{
	...
	window.open('/correction/correct/correct_speaking_appeal.do?id=' + v_id,'correct_appeal',options);	
}

function click_script()
{
	var options = 'toolbar=no,location=no,width=1020,height=760,resizable=no,scrollbars=no,status=no';
	window.open('/correction/correct/correct_speaking_script.do?id=' + $("#correction_exams_answer_id").val(),'correct_script',options);
}